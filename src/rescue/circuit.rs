...
fn pow<E: Engine, CS: ConstraintSystem<E>>(mut cs: CS, x_val: &Option<E::Fr>, x: &Variable, alpha: u32) -> Result<(Option<E::Fr>, Variable), SynthesisError> {
	if alpha == 0 {
		...
		for i in (0..len).rev() {
			y_val = y_val.map(|mut y| { y.square(); y });
			...
			if alpha & (1 << i) != 0 {
				y_val = y_val.and_then(|mut y| x_val.as_ref().map(|x| { y.mul_assign(x); y }));
...
fn inv_pow<E: Engine, CS: ConstraintSystem<E>>(mut cs: CS, x_val: &Option<E::Fr>, x: &Variable, alpha: u32, alpha_inv: &[u64]) -> Result<(Option<E::Fr>, Variable), SynthesisError> {
...
fn vec_add<E: Engine, CS: ConstraintSystem<E>>(mut cs: CS, x: &Vec<(Option<E::Fr>, Variable)>, y: &Vec<(Option<E::Fr>, Variable)>) -> Result<Vec<(Option<E::Fr>, Variable)>, SynthesisError> {
	if x.len() == y.len() {
		x.iter().zip(y.iter()).map(|((x_val, x), (y_val, y))| {
			let z_val = x_val.as_ref().and_then(|x| y_val.as_ref().map(|y| {
				let mut x = *x;
				x.add_assign(y);
				x
			}));
...
fn vec_add_const<E: Engine, CS: ConstraintSystem<E>>(mut cs: CS, x: &Vec<(Option<E::Fr>, Variable)>, y: &Vec<E::Fr>) -> Result<Vec<(Option<E::Fr>, Variable)>, SynthesisError> {
	if x.len() == y.len() {
...
fn vec_pow<E: Engine, CS: ConstraintSystem<E>>(mut cs: CS, x: &Vec<(Option<E::Fr>, Variable)>, alpha: u32) -> Result<Vec<(Option<E::Fr>, Variable)>, SynthesisError> {
	x.iter().map(|(x_val, x)| {
		pow(cs.namespace(|| "pow"), x_val, x, alpha)
	}).collect()
}

fn vec_inv_pow<E: Engine, CS: ConstraintSystem<E>>(mut cs: CS, x: &Vec<(Option<E::Fr>, Variable)>, alpha: u32, alpha_inv: &[u64]) -> Result<Vec<(Option<E::Fr>, Variable)>, SynthesisError> {
	x.iter().map(|(x_val, x)| {
		inv_pow(cs.namespace(|| "inv_pow"), x_val, x, alpha, alpha_inv)
	}).collect()
}

fn mat_vec_mul<E: Engine, CS: ConstraintSystem<E>>(mut cs: CS, mat: &Vec<Vec<E::Fr>>, x: &Vec<(Option<E::Fr>, Variable)>) -> Result<Vec<(Option<E::Fr>, Variable)>, SynthesisError> {
	mat.iter().map(|row| {
		if row.len() == x.len() {
			let y_val = x.iter().map(|(x_val, _)| x_val).zip(row.iter()).map(|(x, y)| x.as_ref().map(|x| {
				let mut x = *x;
				x.mul_assign(y);
				x
			})).fold(Some(E::Fr::zero()), |x, y| x.and_then(|mut x| y.map(|y| {
				x.add_assign(&y);
				x
			})));
			let y = cs.alloc(|| "y", || y_val.ok_or(SynthesisError::AssignmentMissing))?;
